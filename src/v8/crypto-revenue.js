...
function enrichResearch(base, h1, h4, book) {
  ...
  const higherBoost = (h1?.trend ? 9 : h1?.downTrend ? -10 : 0) + (h4?.trend ? 8 : h4?.downTrend ? -9 : 0) + (h1?.trendSlope ? 3 : 0) + (h4?.trendSlope ? 2 : 0);
  const bookBoost = depth.imbalance >= 0.2 ? 10 : depth.imbalance >= 0.1 ? 6 : depth.imbalance <= -0.2 ? -11 : depth.imbalance <= -0.1 ? -5 : 0;
  const velocityBoost = clamp(((base.s1?.ret3 || 0) * 800) + ((base.s5?.ret3 || 0) * 400), -12, 16);
  ...
  const longConfirmed = Boolean(
    ...
    depth.imbalance > -0.2 && !h4?.downTrend && (h1?.trend || h4?.trend || base.trendVotes >= 5)
  );
  return {
    ...base,
    score,
...
function applyCrossMarketConfirmation(signals) {
  ...
  return signals.map(signal => {
    ...
    const crossBoost = clamp((confirmedPeers - 1) * 3 + Math.max(0, averageScore - 70) * 0.08, 0, 10);
...
function cryptoRegime(signals) {
  ...
  return { mode: shock ? "shock" : bullish >= 2 ? "bull" : bearish >= 2 ? "bear" : "mixed", shock };
}

function requiredScore(regime, env) {
  const base = num(env.CRYPTO_MIN_ENTRY_SCORE, 82);
  if (regime.mode === "bull") return Math.max(78, base - 2);
  if (regime.mode === "bear") return Math.max(92, base + 10);
  if (regime.mode === "shock") return Math.max(96, base + 14);
  return Math.max(82, base);
}
...
function entryEconomics(signal, env) {
  ...
  const expectedGrossMove = clamp(Math.max(
    (signal?.atrPct || 0.006) * 1.8,
    Math.max(0, signal?.s5?.ret3 || 0) * 1.5,
    Math.max(0, signal?.s15?.ret1 || 0) * 1.25,
    0.008
  ), 0.008, 0.06);
...
async function placeCryptoBuy(env, candidate, asset, notional, now) {
  ...
  return alpaca(env, "/v2/orders", {
    method: "POST",
    body: JSON.stringify({ symbol: candidate.symbol, notional: round(notional, 2).toFixed(2), side: "buy", type: "limit", limit_price: limitPrice, time_in_force: "ioc", client_order_id: cid("buy", candidate.symbol, now, candidate.s5?.pullback ? "pb" : "trend") })
  });
}
...
export async function runCryptoCycle(env, scheduledTime) {
  ...
  const maxPositions = int(env.CRYPTO_MAX_CONCURRENT_POSITIONS, 4);
  const maxTotal = num(env.CRYPTO_MAX_TOTAL_EXPOSURE_USD, 4000);
  ...
  for (const candidate of ranked) {
    ...
    const notional = Math.min(num(env.CRYPTO_ORDER_NOTIONAL_USD, 1000), num(env.CRYPTO_MAX_POSITION_USD, 1000), remaining, cashLike);
