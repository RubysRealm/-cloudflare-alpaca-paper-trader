...
export async function cryptoOpportunityDiagnostics(env,now=Date.now()){
  ...
  return{strategy:CRYPTO_STRATEGY,readOnly:true,universeCount:symbols.length,qualifiedCount:qualified.length,equity:round(equity,2),cash:round(cash,2),currentExposure:round(currentExposure,2),availableExposure:round(availableExposure,2),maxPositions:maxPos,heldSymbols:[...heldSet],cooldownSymbols:[...cooldowns],dailyDrawdownPct:round(dd.drawdown,5),dailyDrawdownLimitPct:round(dd.max,5),dailyDrawdownBlocked:dd.blocked,recoveryModeEnabled:recoveryEnabled,recoveryModeActive:recoveryEnabled&&dd.blocked,recoveryLossLocked:lossLock.blocked,recoveryLossCooldownMinutesRemaining:round(lossLock.minutesRemaining,1),currentV23OpenSymbols:[...currentOpen],crossVenueProvider:'coinbase_exchange',crossVenueRequired:true,crossVenuePreviewSymbol:previewCandidate?.symbol||null,crossVenuePreview,liquiditySizingPolicy:'quote_depth_first_venue_volume_fallback',candidates:ranked};
}
