...
export const CRYPTO_STRATEGY='free-tier-crypto-v21';
export const CRYPTO_PREFIX='papercrypto-v21-';
...
async function recentBars(env,symbols,now){
  ...
  const start=new Date(Number(now)-22*60000).toISOString();
  const d=await marketDataRaw(env,`/v1beta3/crypto/us/bars?symbols=${symbols.map(encodeURIComponent).join(',')}&timeframe=1Min&start=${encodeURIComponent(start)}&limit=500&sort=asc`);
  return d?.bars||{};
}
...
function barResearch(arr=[]){
  ...
  return{historyOk:xs.length>=12,barCount:xs.length,ret5:ret(5),ret15:ret(15),positive5,volumeExpansion};
}
...
async function buy(env,c,asset,notional,now){
  ...
  return alpaca(env,'/v2/orders',{method:'POST',body:JSON.stringify({symbol:c.symbol,qty,side:'buy',type:'limit',limit_price:String(limit),time_in_force:'ioc',client_order_id:cid('buy',c.symbol,now,'mtf')})});
}
...
function researchCandidates(env,prelim,barsBySymbol){
  ...
  const evaluated=prelim.map(x=>{
    const r=barResearch(barsBySymbol[x.symbol]||[]),projectedMove=Math.max(Math.max(0,x.minRet)*2.5,Math.max(0,r.ret5)*1.35,Math.max(0,r.ret15)*0.45),cost=fee+fullSlip+x.spread,net=projectedMove-cost;
    ...
    const activity=Math.max(10,x.dollarVolume,x.quoteNotional*10),volumeBonus=clamp((r.volumeExpansion-1)*0.45,-0.4,1.2),score=projectedMove*Math.log10(activity)*60+net*300+r.ret5*350+r.ret15*70+r.positive5*0.25+volumeBonus+(x.mega? -0.75:0);
    const reasons=[];
  ...
  return{qualified:evaluated.filter(x=>x.pass),evaluated,researched:evaluated.slice(0,8)};
}

async function buildResearch(env,light,now){
  const prelim=preliminaryCandidates(env,light),shortlist=prelim.slice(0,int(env.CRYPTO_DEEP_RESEARCH_CANDIDATES,10)),bars=await recentBars(env,shortlist.map(x=>x.symbol),now);
...
export async function cryptoOpportunityDiagnostics(env,now=Date.now()){
  const allAssets=await assets(env),symbols=allAssets.map(a=>a.symbol),[positions,account,orders]=await Promise.all([alpaca(env,'/v2/positions'),alpaca(env,'/v2/account'),alpaca(env,'/v2/orders?status=all&limit=200&direction=desc&nested=false')]);
  ...
  const held=(positions||[]).filter(p=>byNorm.has(norm(p.symbol))&&Math.abs(+p.market_value||(+p.qty||0)*(+p.current_price||0))>1),heldSet=new Set(held.map(p=>norm(p.symbol))),cooldowns=cooldownSymbols(env,orders,now),maxPos=int(env.CRYPTO_MAX_CONCURRENT_POSITIONS,2),dd=drawdownState(env,account);
  ...
  const ranked=evaluated.slice(0,12).map(c=>{const s=sizingCaps(env,c,equity,cash,availableExposure),blocks=[...c.reasons];if(heldSet.has(norm(c.symbol)))blocks.push('already_held');if(cooldowns.has(norm(c.symbol)))blocks.push('reentry_cooldown');if(held.length>=maxPos)blocks.push('max_positions');if(dd.blocked)blocks.push('daily_drawdown_circuit_breaker');if(s.notional<minOrder)blocks.push('order_below_min_after_liquidity_cap');return{...diag(c),bid:round(c.bid,8),ask:round(c.ask,8),targetEquityNotional:round(equity*s.targetFraction,2),depthCap:round(s.depthCap,2),volumeCap:round(s.volumeCap,2),liquiditySource:s.liquiditySource,liquidityCap:round(s.liquidityCap,2),finalOrderNotional:round(s.notional,2),cooldown:cooldowns.has(norm(c.symbol)),eligible:c.pass&&!heldSet.has(norm(c.symbol))&&!cooldowns.has(norm(c.symbol))&&held.length<maxPos&&!dd.blocked&&s.notional>=minOrder,blocks:[...new Set(blocks)]};});
  const sample=sizingCaps(env,{score:0,quoteNotional:0,dollarVolume:0},equity,cash,availableExposure);
  return{strategy:CRYPTO_STRATEGY,readOnly:true,universeCount:symbols.length,qualifiedCount:qualified.length,equity:round(equity,2),cash:round(cash,2),currentExposure:round(currentExposure,2),availableExposure:round(availableExposure,2),maxPositions:maxPos,heldSymbols:[...heldSet],cooldownSymbols:[...cooldowns],dailyDrawdownPct:round(dd.drawdown,5),dailyDrawdownLimitPct:round(dd.max,5),dailyDrawdownBlocked:dd.blocked,bookParticipation:sample.bookParticipation,dailyVolumeParticipation:sample.dailyParticipation,liquiditySizingPolicy:'quote_depth_first_venue_volume_fallback',candidates:ranked};
}
...
export async function runCryptoFreeTier(env,now,{discover=true}={}){
  ...
  const byNorm=new Map(allAssets.map(a=>[norm(a.symbol),a])),held=(positions||[]).filter(p=>byNorm.has(norm(p.symbol))&&Math.abs(+p.market_value||(+p.qty||0)*(+p.current_price||0))>1),maxPos=int(env.CRYPTO_MAX_CONCURRENT_POSITIONS,2),dd=drawdownState(env,account);
  if(dd.blocked)return{status:actions.length?'acted':'hold',strategy:CRYPTO_STRATEGY,mode:'risk_circuit_breaker',reason:'daily_drawdown_limit',dailyDrawdownPct:round(dd.drawdown,5),dailyDrawdownLimitPct:round(dd.max,5),qualified:finalists.map(diag),researched:researched.map(diag),actions};
  ...
  const heldSet=new Set(held.map(p=>norm(p.symbol))),cooldowns=cooldownSymbols(env,orders,now),c=finalists.find(x=>!heldSet.has(norm(x.symbol))&&!cooldowns.has(norm(x.symbol)));
  ...
  const cash=Math.max(0,+(account.non_marginable_buying_power||account.cash||account.buying_power||0)),equity=Math.max(cash,+(account.equity||account.portfolio_value||0)),configuredTotalCap=num(env.CRYPTO_MAX_TOTAL_EXPOSURE_USD,50000),currentExposure=held.reduce((z,p)=>z+Math.abs(+p.market_value||(+p.qty||0)*(+p.current_price||0)),0),totalCap=Math.min(configuredTotalCap,equity*0.50),availableExposure=Math.max(0,totalCap-currentExposure),s=sizingCaps(env,c,equity,cash,availableExposure),asset=byNorm.get(norm(c.symbol));
  if(s.notional>=num(env.CRYPTO_MIN_ORDER_NOTIONAL_USD,25)){
    try{const o=await buy(env,c,asset,s.notional,now);if(o)actions.push({action:'crypto_buy',symbol:c.symbol,reason:'v21_multitimeframe_persistent_velocity',minuteMove:round(c.minRet,5),fiveMinuteMove:round(c.ret5,5),fifteenMinuteMove:round(c.ret15,5),positiveBars5:c.positive5,volumeExpansion:round(c.volumeExpansion,3),dayMove:round(c.dayRet,5),expectedNetEdge:round(c.net,5),score:round(c.score,4),equityFraction:round(s.targetFraction,4),liquiditySource:s.liquiditySource,notional:round(s.notional,2),orderStatus:o?.status||null,orderType:o?.type||null});}
