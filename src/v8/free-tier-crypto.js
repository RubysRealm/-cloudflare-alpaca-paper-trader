...
export const CRYPTO_STRATEGY='free-tier-crypto-v23';
export const CRYPTO_PREFIX='papercrypto-v23-';
...
function researchCandidates(env,prelim,barsBySymbol){
  ...
  const evaluated=prelim.map(x=>{
    ...
    const activity=Math.max(10,x.dollarVolume,x.quoteNotional*10),score=projectedMove*Math.log10(activity)*60+net*300+r.ret5*350+r.ret15*80+r.ret30*30+r.positive5*0.25+(x.mega?-0.75:0);
    const reasons=[];
...
function recoverySizing(env,c,equity,cash,availableExposure){
  const fraction=clamp(pct(env.CRYPTO_RECOVERY_MAX_POSITION_PCT,0.06),0.02,0.15),bookParticipation=clamp(num(env.CRYPTO_RECOVERY_MAX_BOOK_PARTICIPATION,0.12),0.05,0.20),configuredCap=num(env.CRYPTO_RECOVERY_MAX_POSITION_USD,7500);
...
export async function cryptoOpportunityDiagnostics(env,now=Date.now()){
  ...
  const held=(positions||[]).filter(p=>byNorm.has(norm(p.symbol))&&Math.abs(+p.market_value||(+p.qty||0)*(+p.current_price||0))>1),heldSet=new Set(held.map(p=>norm(p.symbol))),cooldowns=cooldownSymbols(env,orders,now),maxPos=int(env.CRYPTO_MAX_CONCURRENT_POSITIONS,2),dd=drawdownState(env,account),rawCurrentOpen=currentStrategyOpenSymbols(orders),currentOpen=new Set([...rawCurrentOpen].filter(k=>heldSet.has(k))),lossLock=recoveryLossLock(env,orders,now),recoveryEnabled=String(env.CRYPTO_RECOVERY_MODE_ENABLED??'true')==='true';
  ...
  const previewCandidate=dd.blocked?qualified.find(c=>recoveryQualifies(env,c)&&!heldSet.has(norm(c.symbol))&&!cooldowns.has(norm(c.symbol))):qualified.find(c=>!heldSet.has(norm(c.symbol))&&!cooldowns.has(norm(c.symbol)));
  const crossVenuePreview=previewCandidate?await confirmCoinbaseMomentum(env,previewCandidate,now):null;
  const ranked=evaluated.slice(0,12).map(c=>{
    ...
    const localRecovery=recoveryEnabled&&dd.blocked&&recoveryQualifies(env,c)&&!heldSet.has(norm(c.symbol))&&!cooldowns.has(norm(c.symbol))&&currentOpen.size===0&&!lossLock.blocked&&r.notional>=minOrder;
    const previewMatch=crossVenuePreview&&previewCandidate&&norm(previewCandidate.symbol)===norm(c.symbol),recoveryPass=localRecovery&&previewMatch&&crossVenuePreview.pass===true;
    return{...diag(c),bid:round(c.bid,8),ask:round(c.ask,8),targetEquityNotional:round(equity*s.targetFraction,2),depthCap:round(s.depthCap,2),liquiditySource:s.liquiditySource,liquidityCap:round(s.liquidityCap,2),finalOrderNotional:round(s.notional,2),cooldown:cooldowns.has(norm(c.symbol)),eligible:c.pass&&!heldSet.has(norm(c.symbol))&&!cooldowns.has(norm(c.symbol))&&held.length<maxPos&&!dd.blocked&&s.notional>=minOrder,recoveryLocalEligible:localRecovery,recoveryEligible:recoveryPass,recoveryOrderNotional:round(r.notional,2),blocks:[...new Set(blocks)]};
  });
  return{strategy:CRYPTO_STRATEGY,readOnly:true,universeCount:symbols.length,qualifiedCount:qualified.length,equity:round(equity,2),cash:round(cash,2),currentExposure:round(currentExposure,2),availableExposure:round(availableExposure,2),maxPositions:maxPos,heldSymbols:[...heldSet],cooldownSymbols:[...cooldowns],dailyDrawdownPct:round(dd.drawdown,5),dailyDrawdownLimitPct:round(dd.max,5),dailyDrawdownBlocked:dd.blocked,recoveryModeEnabled:recoveryEnabled,recoveryModeActive:recoveryEnabled&&dd.blocked,recoveryLossLocked:lossLock.blocked,recoveryLossCooldownMinutesRemaining:round(lossLock.minutesRemaining,1),currentV23OpenSymbols:[...currentOpen],rawV23OrderInventorySymbols:[...rawCurrentOpen],actualPositionSourceOfTruth:true,crossVenueProvider:'coinbase_exchange',crossVenueRequired:true,crossVenuePreviewSymbol:previewCandidate?.symbol||null,crossVenuePreview,liquiditySizingPolicy:'quote_depth_first_venue_volume_fallback',candidates:ranked};
}
...
export async function runCryptoFreeTier(env,now,{discover=true}={}){
  ...
  if(dd.blocked){
    const recoveryEnabled=String(env.CRYPTO_RECOVERY_MODE_ENABLED??'true')==='true',rawCurrentOpen=currentStrategyOpenSymbols(orders),currentOpen=new Set([...rawCurrentOpen].filter(k=>heldSet.has(k))),lossLock=recoveryLossLock(env,orders,now),c=finalists.find(x=>recoveryQualifies(env,x)&&!heldSet.has(norm(x.symbol))&&!cooldowns.has(norm(x.symbol)));
    ...
    const crossVenue=await confirmCoinbaseMomentum(env,c,now);
    if(!crossVenue.pass)return{status:actions.length?'acted':'hold',strategy:CRYPTO_STRATEGY,mode:'risk_recovery_wait',reason:'cross_venue_confirmation_failed',candidate:diag(c),crossVenue,actions};
    const r=recoverySizing(env,c,equity,cash,availableExposure),asset=byNorm.get(norm(c.symbol));
    if(r.notional<minOrder)return{status:actions.length?'acted':'hold',strategy:CRYPTO_STRATEGY,mode:'risk_recovery_wait',reason:'recovery_liquidity_too_thin',candidate:diag(c),recoveryOrderNotional:round(r.notional,2),crossVenue,actions};
    try{const o=await buy(env,c,asset,r.notional,now,'recovery');if(o)actions.push({action:'crypto_buy',symbol:c.symbol,reason:'v23_recovery_cross_venue_confirmed',minuteMove:round(c.minRet,5),fiveMinuteMove:round(c.ret5,5),fifteenMinuteMove:round(c.ret15,5),thirtyMinuteMove:round(c.ret30,5),positiveBars5:c.positive5,dayMove:round(c.dayRet,5),expectedNetEdge:round(c.net,5),score:round(c.score,4),crossVenue,recoveryMode:true,equityFraction:round(r.fraction,4),bookParticipation:round(r.bookParticipation,4),depthCap:round(r.depthCap,2),notional:round(r.notional,2),orderStatus:o?.status||null,orderType:o?.type||null});}
    catch(e){actions.push({action:'crypto_buy_failed',symbol:c.symbol,reason:e.message,recoveryMode:true,crossVenue});}
    return{status:actions.some(a=>a.action==='crypto_buy'||a.action==='crypto_sell')?'acted':'hold',strategy:CRYPTO_STRATEGY,mode:'controlled_recovery',dailyDrawdownPct:round(dd.drawdown,5),dailyDrawdownLimitPct:round(dd.max,5),qualified:finalists.map(diag),crossVenue,actions};
  }
  ...
  const crossVenue=await confirmCoinbaseMomentum(env,c,now);
  if(!crossVenue.pass)return{status:actions.length?'acted':'hold',strategy:CRYPTO_STRATEGY,mode:'multitimeframe_velocity_rank',reason:'cross_venue_confirmation_failed',candidate:diag(c),crossVenue,actions};
  const s=sizingCaps(env,c,equity,cash,availableExposure),asset=byNorm.get(norm(c.symbol));
  if(s.notional>=minOrder){
    try{const o=await buy(env,c,asset,s.notional,now,'mtf');if(o)actions.push({action:'crypto_buy',symbol:c.symbol,reason:'v23_multitimeframe_cross_venue_confirmed',minuteMove:round(c.minRet,5),fiveMinuteMove:round(c.ret5,5),fifteenMinuteMove:round(c.ret15,5),thirtyMinuteMove:round(c.ret30,5),positiveBars5:c.positive5,dayMove:round(c.dayRet,5),expectedNetEdge:round(c.net,5),score:round(c.score,4),crossVenue,equityFraction:round(s.targetFraction,4),liquiditySource:s.liquiditySource,notional:round(s.notional,2),orderStatus:o?.status||null,orderType:o?.type||null});}
    catch(e){actions.push({action:'crypto_buy_failed',symbol:c.symbol,reason:e.message,crossVenue});}
  }else actions.push({action:'crypto_buy_skipped',symbol:c.symbol,reason:'insufficient_executable_liquidity_for_min_order',liquiditySource:s.liquiditySource,liquidityCap:round(s.liquidityCap,2),crossVenue});
  return{status:actions.some(a=>a.action==='crypto_buy'||a.action==='crypto_sell')?'acted':'hold',strategy:CRYPTO_STRATEGY,mode:'multitimeframe_velocity_rank',universeCount:symbols.length,qualified:finalists.map(diag),researched:researched.map(diag),cooldownSymbols:[...cooldowns],crossVenue,actions};
}

export function cryptoFreeTierStatus(env){
  return{strategy:CRYPTO_STRATEGY,endpoint:'paper',market:'24x7',entryOrderType:'protected_limit_ioc',hardStopExitOrderType:'market',profitExitOrderType:'protected_limit_ioc',positionPnlSource:'executable_bid_only',liquiditySizingPolicy:'quote_depth_first_venue_volume_fallback',crossVenueProvider:'coinbase_exchange',crossVenueConfirmationEnabled:String(env.CRYPTO_CROSS_VENUE_CONFIRM_ENABLED??'true')==='true',freeTier:{cpuMsPerInvocation:10,requestLimitPerDay:100000,architecture:'all_tradable_usd_crypto_multitimeframe_velocity_rank',finalists:int(env.FREE_TIER_CRYPTO_FINALISTS,6),alternatingMarketDiscovery:false},research:{allActiveTradablePairs:true,stablecoinsExcluded:true,fullExecutableUsdUniverse:true,percentageVelocityPriority:true,multiTimeframeConfirmation:true,persistenceConfirmation:true,thirtyMinuteContext:true,volumeExpansionResearch:false,crossVenueConfirmation:true,crossVenueRealTradeCandles:true,crossVenueTickerLiquidity:true,crossVenuePriceSanity:true,actualPositionSourceOfTruth:true,megaCapDeprioritization:true,quoteDepthLiquidity:true,venueVolumeFallbackLiquidity:true,candidateDiagnostics:true,staleOrderCancellation:true,hardStopExecutableOnly:true,singleMinuteExitDisabled:true,minHoldBeforeNoiseExit:true,rotationRequiresNetProfit:true,dailyDrawdownCircuitBreaker:true,controlledRecoveryMode:true,recoveryLossLockout:true,recoveryRequiresHighConviction:true,qualityOverTurnover:true,liquidityAwarePositionSizing:true,equityScaledPositionSizing:true,maxPortfolioExposurePct:0.50,timeframes:['1Min','5Min','15Min','30Min','1Day'],crossSectionRelativeStrength:true,researchBeforeExecution:true,tradeVolumeObjective:false,antiChaseEntryTiming:true},takeProfitPct:pct(env.CRYPTO_TAKE_PROFIT_PCT,0.0125),minHoldMinutes:int(env.CRYPTO_MIN_HOLD_MINUTES,5),reentryCooldownMinutes:int(env.CRYPTO_REENTRY_COOLDOWN_MINUTES,20),hardStopReentryCooldownMinutes:int(env.CRYPTO_HARDSTOP_REENTRY_COOLDOWN_MINUTES,60),dailyDrawdownLimitPct:pct(env.CRYPTO_MAX_DAILY_DRAWDOWN_PCT,0.02),recoveryModeEnabled:String(env.CRYPTO_RECOVERY_MODE_ENABLED??'true')==='true',recoveryMaxPositionPct:pct(env.CRYPTO_RECOVERY_MAX_POSITION_PCT,0.06),recoveryLossCooldownMinutes:int(env.CRYPTO_RECOVERY_LOSS_COOLDOWN_MINUTES,180),recoveryMin5mPct:pct(env.CRYPTO_RECOVERY_MIN_5M_MOMENTUM_PCT,0.006),recoveryMin15mPct:pct(env.CRYPTO_RECOVERY_MIN_15M_MOMENTUM_PCT,0.008),recoveryMinNetEdgePct:pct(env.CRYPTO_RECOVERY_MIN_NET_EDGE_PCT,0.004),coinbaseMin5mPct:pct(env.CRYPTO_COINBASE_MIN_5M_MOMENTUM_PCT,0.003),coinbaseMin15mPct:pct(env.CRYPTO_COINBASE_MIN_15M_MOMENTUM_PCT,0.005),coinbaseMin30mDollarVolume:num(env.CRYPTO_COINBASE_MIN_30M_DOLLAR_VOLUME_USD,100000),maxCrossVenuePriceDivergencePct:pct(env.CRYPTO_MAX_CROSS_VENUE_PRICE_DIVERGENCE_PCT,0.012),maxBookParticipation:clamp(num(env.CRYPTO_MAX_BOOK_PARTICIPATION,0.20),0.05,0.35),maxDailyVolumeParticipation:clamp(num(env.CRYPTO_MAX_DAILY_VOLUME_PARTICIPATION,0.01),0.001,0.05)};
}
